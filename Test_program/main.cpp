    ...
    for (size_t i = 0; i < num_images; i++)
    {
      ...
      //  imshow("window",gray);
       // waitKey(0);
        ...
        bool patternfound = findChessboardCorners(gray, patternsize, centers,CALIB_CB_ADAPTIVE_THRESH + CALIB_CB_NORMALIZE_IMAGE
                                                  + CALIB_CB_FAST_CHECK);
        if (patternfound)
        {
            cornerSubPix(gray, centers, Size(11, 11), Size(-1, -1),
                                 TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 30, 0.1));
...
//    const Mat robot_tr_10 = (Mat_<float>(3, 1) << 813.18, 400.44, 917.16);
    ...
    //Part 2
    const Mat robot_tr_01 = (Mat_<float>(3, 1) << -941.7910, 8.6871, 120.6610);
    const Mat robot_tr_02 = (Mat_<float>(3, 1) << -947.4020, -153.0410, 101.1080);
    const Mat robot_tr_03 = (Mat_<float>(3, 1) << -950.4640, -150.7550, 537.3240);
    const Mat robot_tr_04 = (Mat_<float>(3, 1) << -944.2820, 22.7132, 536.4470);
    const Mat robot_tr_05 = (Mat_<float>(3, 1) << -1197.3, 69.3, 335.5);
    const Mat robot_tr_06 = (Mat_<float>(3, 1) << -1076.7, -104.6, 323.4);
    const Mat robot_tr_07 = (Mat_<float>(3, 1) << -1162.7, -110.8, 396.7);
    const Mat robot_tr_08 = (Mat_<float>(3, 1) << -1073.8, -128.3, 438.5);
    const Mat robot_tr_09 = (Mat_<float>(3, 1) << -1073.8, -128.3, 438.5);
    const Mat robot_tr_10 = (Mat_<float>(3, 1) << -1171.5, -166.3, 630.0);
    ...
    cout << t_cam2gripper.size() << " t_cam2gripper"<<endl;



    calibrateHandEye(R_gripper2base, t_gripper2base, R_target2cam, t_target2cam, R_cam2gripper, t_cam2gripper, CALIB_HAND_EYE_HORAUD);
...
Vec3f rotationMatrixToEulerAngles(Mat &R)
{
    ...
    return Vec3f(rad2deg(z), rad2deg(y), rad2deg(x));
